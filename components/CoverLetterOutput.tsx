...
export default function CoverLetterOutput({
  ...
  const { user, isPro, creditsBalance, subscription } = useAuth();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <UpgradePrompt source="pdf" compact isPro={isPro} userEmail={user?.email} userId={user?.id} creditsBalance={creditsBalance} subscription={subscription} />
          <UpgradePrompt source="optimize" compact isPro={isPro} userEmail={user?.email} userId={user?.id} creditsBalance={creditsBalance} subscription={subscription} />
          <button
            onClick={handleCopy}
